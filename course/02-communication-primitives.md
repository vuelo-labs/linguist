...
> **Footnote (as of Claude Code v2.1.128, 2026-05-05).** The verb still does the work, but Claude Code 2.1.x adds two affordances that change *who chooses the verb*. **Auto mode** (Mar 25, 2026) skips per-step permission prompts inside a permitted scope, so an ambiguous verb runs further before you see it; the cost of `improve` rises accordingly. **Skills with proactive activation** (SKILL.md frontmatter) let the harness invoke a skill on a matched intent — meaning the *skill's* verbs, not yours, drive the action once it activates. In both cases the discipline is the same: be unambiguous up front, because there are fewer interrupts on the way down.
